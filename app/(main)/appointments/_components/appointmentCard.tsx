...
import { BookingProps } from './AppointmentList'
import { Card, CardContent, CardHeader } from '@/components/ui/card'
import { Avatar, AvatarFallback, AvatarImage } from '@/components/ui/avatar'
import { Badge } from '@/components/ui/badge'
import { Separator } from '@/components/ui/separator'
import { ArrowRight, Calendar, Clock, Sparkles, Tv, Video } from 'lucide-react'
import { formatDuration } from '@/lib/helpers'
import { Button } from '@/components/ui/button'
import Link from 'next/link'
import { RATING_CONFIG, RATING_LABEL } from '@/lib'
...
const AppointmentCard = ({booking,isPast,mode}:AppointmentCardProps) => {
    ...
    const {interviewer,startTime,endTime,status,feedback, streamCallId, creditsCharged,recordingUrl}=booking

    const isUpcoming=status==="SCHEDULED"&&new Date(startTime)
    ...
    <div className='flex items-start gap-2 justify-between'>
        <div className='flex items-center gap-3'>
            <Avatar className='w-16 h-16 rounded-md border border-amber-600/30'>
                <AvatarImage className='rounded-md ' src={interviewer.imageUrl||undefined} />
                <AvatarFallback className='rounded-md bg-amber-400/10 text-amber-400 border-none text-xl'>{interviewer.name?.charAt(0)}</AvatarFallback>
            </Avatar>
            <div className="flex flex-col">
                <h3 className="text-base font-semibold">{interviewer.name}</h3>
                <p className="text-muted-foreground text-xs">{interviewer.title} <span> . </span> {interviewer.company}</p>
              <div className="flex items-center gap-2 overflow-x-auto flex-wrap mt-1">
                  {
                    interviewer.categories.length>0&&(
                        interviewer.categories.map(c=>(
...
<CardContent>
...

    {
        feedback&&(
            ...
            {/*  feedback model */}
            <FeedbackModel feedback={feedback} feedbackOpened={feedbackOpened} setFeedbackOpened={setFeedbackOpened} intervieweeName={interviewer.name||undefined}/>
            </div>
        )
    }
